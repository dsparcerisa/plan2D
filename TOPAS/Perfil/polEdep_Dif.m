...
[SEdep,SEdep_weighted,NR,NZ,dR,dZ,spreadX,spreadY,angularspreadX,angularspreadY,energy,N_histories] = loadEdep_Dif(SimulationNumber);
%%
Direction = sprintf('Difusor/%iMeV/Sim%i/Images/',energy,SimulationNumber);
etiqueta = sprintf('_Dif_%iMeV_Sim%i',energy,SimulationNumber);
SEdep_weighted_error = SEdep_weighted(2,:);
SEdep_weighted = SEdep_weighted(1,:);
RMaxValues = dR * (1:NR);
RMinValues = RMaxValues - dR;
RValues = RMaxValues - dR/2;
Zo = 3; %HAY QUE DESPLAZAR EL SCORER XQ NO EMPIEZA A LA SALIDA DEL HAZ!!!!!!!
ZMaxValues = dZ * (1:NZ);
ZMinValues = ZMaxValues - dZ;
ZValues = ZMaxValues - dZ/2 + Zo;
...
%% Fit and plot  
    ...
    Directory = sprintf('Difusor/%iMeV/Sim%i',energy,SimulationNumber);
    ...
    save([Directory,'/Results.mat'],'polyEdep','nlm','ZValues','RValues','SEdep','SEdep_weighted','SEdep_weighted_error')
    copyfile('CON_DIFUSOR.txt',Directory);
    copyfile('Edep_Dif.csv',Directory);
    copyfile('Edep_STD_Dif.csv',Directory);
